...
class Rando_Player():
	...
	def assign_utility(self,board,old_move,flag):
		...
		temp=[['-' for i in range(4)] for j in range(4)]
		#temp_board=Board();
		#temp_board.block_status=copy.deepcopy(board.block_status);
		#temp_board.board_status=copy.deepcopy(board.board_status);
		heuristic=0;
		cntx = 0
		cnto = 0
		cntd = 0
		#temp_board.update(old_move,(1,1),'x');
		...
		for block_row in range(4):
			for block_col in range(4):
				...
				if block[block_row][block_col] == '-':
					for i in range(4):
						for j in range(4):
							temp[i][j]=b[block_row*4+i][block_col*4+j]
					#print temp;
					for i in range(4):
						row = temp[i]							#i'th row
						col = [x[i] for x in temp]			#i'th column
						#print row,col
						if row.count('o')==0:
							temp_heuristic+=10*row.count('x');
						if row.count('x')==0:
							temp_heuristic-=10*row.count('o');
						if col.count('o')==0:
							temp_heuristic+=10*col.count('x');
						if col.count('x')==0:
							temp_heuristic-=10*col.count('o');
	...
	def minmax(self,board,old_move,flag):
		...
		for i in range(total_possible_ways):
			#print cells[i];
			new_move=cells[i];
			self.min_step(board,old_move,new_move,depth,flag);



	def min_step(self,board,old_move,new_move,depth,flag):
		if depth==0:
			return
		temp_board=Board();
		temp_board.block_status=copy.deepcopy(board.block_status);
	...
	def max_step(self,board,old_move,new_move,depth,flag):
		temp_board=Board();
		temp_board.block_status=copy.deepcopy(board.block_status);
		temp_board.board_status=copy.deepcopy(board.board_status);
		#print new_move
		...
		for i in range(total_possible_ways):
			#print i;
			move=cells[i];
			self.min_step(temp_board,new_move,move,0,flag);
	...
	def move(self, board, old_move, flag):
		...
		self.minmax(board,old_move,flag);
		#print cells
		#for i in range(total_possible_ways):
		#	print cells[i];
		temp =cells[random.randrange(len(cells))];
		return temp;
